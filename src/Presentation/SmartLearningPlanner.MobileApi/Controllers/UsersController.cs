...
{          
    ...
    {
        await _userService.ChangePasswordAsync(changePasswordDto.Id, changePasswordDto.CurrentPassword, changePasswordDto.NewPassword);
        return NoContent();
    }
    [HttpGet("check-password")]
    public async Task<ActionResult> CheckPassword(string id, string password)
    {
        var result =  await _userService.CheckPasswordAsync(id, password);
        return Ok(result);
    }
}
